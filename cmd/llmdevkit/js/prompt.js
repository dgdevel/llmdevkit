...
export async function cancelSession() {
    ...
    if (!conv) return;
    await fetch('/api/conversations/' + conv.id + '/cancel', {
        method: 'POST'
    });
    conv.running = false;
    S.running = false;
    S.taskEntries = S.taskEntries.map(t => t.status === 'in_progress' ? {
        ...t,
        status: 'completed'
    } : t);
    renderTaskList();
...
export async function undoLastPrompt() {
    ...
    if (!conv) return;
    if (conv.running) {
        alert('Cannot undo while conversation is running.');
        return;
    }
    ...
    try {
        const r = await fetch('/api/conversations/' + conv.id + '/undo', {
            method: 'POST'
        });
        if (!r.ok) {
...
export async function trimConversation() {
    ...
    if (!conv) return;
    if (conv.running) {
        alert('Cannot trim while conversation is running.');
        return;
    }
    const hasTools = (conv.messages || []).some(m => m.type === 'tool_request' || m.type === 'tool_response');
    if (!hasTools) {
        alert('No tool calls or tool replies to trim.');
        return;
    }
    ...
    try {
        const r = await fetch('/api/conversations/' + conv.id + '/trim', {
            method: 'POST'
        });
        if (!r.ok) {
            const data = await r.json().catch(() => ({}));
