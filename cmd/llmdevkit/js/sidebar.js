...
export function renderConvList() {
  ...
  el.querySelectorAll('.conv-item').forEach(item => {
    ...
    delBtn.addEventListener('click', e => { e.stopPropagation(); deleteConversation(item.dataset.id); });
    const renameBtn = item.querySelector('.conv-rename');
    renameBtn.addEventListener('click', e => {
      e.stopPropagation();
      startRename(item.dataset.id);
    });
    item.addEventListener('mouseenter', () => { renameBtn.style.display = 'inline'; });
    item.addEventListener('mouseleave', () => { renameBtn.style.display = 'none'; });
  });
}
